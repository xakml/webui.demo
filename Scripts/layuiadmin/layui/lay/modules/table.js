    ...
    F.prototype.config = {
        ...
        if (i.startTime = (new Date).getTime(), a.url) {
            ...
            a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), i.loading(), t.ajax({
                type: a.method || "get",
                url: a.url.indexOf('/oms/') > -1 ? (setter.baseOmsServer || '') + a.url.slice(a.url.indexOf('/oms/')) :
                    a.url.indexOf('/waybillinfo/') > -1 ? (setter.baseOmsServer || '') + a.url.slice(a.url.indexOf('/waybillinfo/')) :
                        a.url.indexOf('/baseinfo/') > -1 ? (setter.baseOmsServer || '') + a.url.slice(a.url.indexOf('/baseinfo/')) : a.url,
                contentType: a.contentType,
                data: d,
                dataType: "json",
                headers: a.url.indexOf('/oms/') > -1 ? setter.omsHeader : a.headers || {},
                success: function (t) {
            ...
            g = [], b = function () {
                ...
                return !r && c.sortKey ? c.sort(c.sortKey.field, c.sortKey.sort, !0) : (layui.each(y, function (a, l) {
                    ...
                    0 !== l.length && (r || (l[d.config.indexName] = a), c.eachCols(function (n, r) {
                        var c = r.field || n, v = s.index + "-" + r.key, m = function () {
                            let pValue = l;
                            if (c) {
                                for (const cElement of c.toString().split("\.")
                                    ) {
                                    if (pValue) {
                                        pValue = pValue[cElement]
                                    }
                                }
                            }
                            return pValue
                ...
                "object" == typeof e ? (layui.each(e, function (e, a) {
                    ...
                    if (l.field && "normal" == l.type && !l.hide) {
                        var o = function () {
                            let pValue = a;
                            if (l.field) {
                                for (const cElement of l.field.toString().split("\.")) {
                                    if (pValue) {
                                        pValue = pValue[cElement]
                                    }
                                }
